...
OUT=/space/rwang/fake-citation-detector/scripts/arxiv_sweep_v8b
mkdir -p $OUT
...
while (y,m)<=(26,6):
    ...
    if m>12: m=1;y+=1
N=12; sz=math.ceil(len(months)/N)
...
for p in "${PAIRS[@]}"; do
  ...
  setsid python3 arxiv_sweep.py --tei-source $TEI --start $S --end $E --k 12000 \
    --workers 1 --solr-workers 1 --outdir $OUT >> $OUT/run_${S}_${E}.log 2>&1 < /dev/null &
  sleep 0.5
done
